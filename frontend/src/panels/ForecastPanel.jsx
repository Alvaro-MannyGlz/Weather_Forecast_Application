...
export default function ForecastPanel({ location = "New York", days = 3 }) {
  ...
  async function fetchForecast(loc, d) {
    ...
    try {
      ...
      const url = `${apiBase}/forecast.json?q=${encodeURIComponent(loc)}&days=${d}&aqi=no&alerts=no`;

      console.debug("Forecast fetch url ->", url);
      ...
      console.error("ForecastPanel error:", err);
    } finally {
      setLoading(false);
    }
  }
  ...
  const fmtHour = (raw) => {
    try { return new Date(raw).toLocaleTimeString([], { hour: "numeric" }); }
    catch { return raw; }
  };
  const fmtDate = (raw) => {
    try { return new Date(raw).toLocaleDateString(undefined, { weekday: "short", month: "short", day: "numeric" }); }
    catch { return raw; }
  };
        ...
        <div className="flex gap-3 overflow-x-auto pb-2">
          {loading && !data
            ...
            : (data?.forecast?.forecastday?.[0]?.hour || []).map((h, i) => (
                <div key={i} className="min-w-[88px] flex-shrink-0 bg-slate-50 rounded-lg p-2 text-center">
                  <div className="text-xs text-muted-foreground">{fmtHour(h.time)}</div>
                  <img src={h.condition.icon} alt={h.condition.text} className="mx-auto my-1 w-8 h-8" decoding="async"/>
            ...
            : (data?.forecast?.forecastday || []).map((day, i) => (
                <div key={i} className="p-3 bg-slate-50 rounded-lg flex items-center gap-3">
                  <div className="flex-shrink-0 text-center w-20">
                    ...
                    <img src={day.day.condition.icon} alt={day.day.condition.text} className="mx-auto mt-1 w-10 h-10" decoding="async"/>
