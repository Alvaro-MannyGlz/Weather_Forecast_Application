...
export default function App() {
  ...
  const handleSelectLocation = (loc) => {
    ...
    setSavedLocations((s) => (s.includes(loc) ? s : [loc, ...s]));
  };

  const handleRemoveSaved = (loc) => {
    setSavedLocations((s) => s.filter((x) => x !== loc));
    setCurrentLocation((cur) =>
      cur === loc ? savedLocations.find((x) => x !== loc) || "" : cur
    );
  };

  const handleClickSaved = (loc) => setCurrentLocation(loc);
  ...
  return (
    <div className="mx-auto max-w-7xl space-y-6 p-4">
      ...
      <ForecastPanel location={currentLocation} days={3} />
